...
class SparseOutputLoss(nn.Module):
    def __init__(self, base_loss, l1_weight=0.01):
        ...
        self.base_loss = base_loss  # Original loss function, such as nn.MSELoss()
        self.l1_weight = l1_weight  # L1 regularization weight
...
# L_total = L_original + λ⋅ ∑_i∣w_i∣
class L1Regularizer(_Regularizer):
    """
    L1 regularized loss
    """
    def __init__(self, model, lambda_reg=0.001):
        # Model: neural network model to be regularized lambda_reg: regularization intensity coefficient, default 0.001, the smaller the value, the milder the regularization effect; The larger the value, the stronger the regularization effect
        super(L1Regularizer, self).__init__(model=model)
        self.lambda_reg = lambda_reg

    def regularized_param(self, param_weights, reg_loss_function):
        # Function: apply L1 regularization to a single parameter tensor, param_weights: parameter weights requiring regularization, reg_loss_function: current regularization loss value
        reg_loss_function += self.lambda_reg * L1Regularizer.__add_l1(var=param_weights)
        return reg_loss_function

    def regularized_all_param(self, reg_loss_function):
        # Apply L1 regularization to all weight parameters
